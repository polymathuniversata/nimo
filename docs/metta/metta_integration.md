...
```powershell
# From project root
.\scripts\setup_metta.ps1
```

Or install manually:
```bash
# Create virtual environment
python -m venv metta-env
.\metta-env\Scripts\activate
...
class Config:
    ...
    METTA_CACHE_SIZE = int(os.environ.get('METTA_CACHE_SIZE', '1000'))
```
...
python backend/scripts/run_metta.py backend/tests/simple_test.metta

# Test with REPL
python -c "import hyperon; hyperon.repl()"
```
...
3. **Rule Errors**: Check syntax in `backend/rules/core_rules.metta` file
