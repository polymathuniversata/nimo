...
- **Frontend Application** (Vue.js/Quasar SPA)
...
- **Cardano Mainnet RPC**: Blockfrost API for Cardano network access
- **Cardano Explorer API**: For transaction verification
...
server {
    ...
    add_header Content-Security-Policy "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://cardano-mainnet.blockfrost.io wss://cardano-mainnet.blockfrost.io;";
...
CARDANO_WALLET_SEED=YOUR_WALLET_SEED_PHRASE_HERE
```

### 2.2 Deploy to Cardano Mainnet
...
python scripts/deploy_contracts.py \
  --network mainnet \
  --blockfrost-key $BLOCKFROST_API_KEY \
  --wallet-seed "$CARDANO_WALLET_SEED"

# Save deployment addresses
echo "NIMO_IDENTITY_CONTRACT=$(cat deployments/mainnet/NimoIdentity.addr)" >> ../backend/.env.production
echo "NIMO_TOKEN_POLICY_ID=$(cat deployments/mainnet/NimoToken.policy)" >> ../backend/.env.production
```

### 2.3 Verify Contracts

```bash
# Verify contract deployment on Cardano Explorer
python scripts/verify_deployment.py \
  --network mainnet \
  --contract-address $NIMO_IDENTITY_CONTRACT \
  --policy-id $NIMO_TOKEN_POLICY_ID
```
...
NIMO_TOKEN_POLICY_ID=TOKEN_POLICY_ID_FROM_DEPLOYMENT
CARDANO_WALLET_SEED=YOUR_WALLET_SEED_PHRASE_HERE
...
VITE_NIMO_TOKEN_POLICY_ID=TOKEN_POLICY_ID_FROM_DEPLOYMENT
...
- `GET /health/cardano` - Cardano blockchain connectivity check
...
- [ ] Smart contracts deployed and verified on Cardano Explorer
...
curl -k https://your-domain.com/api/health/cardano
```
...
- Ensure sufficient ADA for transactions
